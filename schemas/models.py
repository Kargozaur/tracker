...
from sqlalchemy import String, Boolean, text, Integer, ForeignKey, DECIMAL
...
class User(Base):
    ...
    workout_plans = relationship("WorkoutPlans", back_populates="user")
...
class Exercise(Base):
    ...
    category_id: Mapped[int] = mapped_column(
        ForeignKey("exercise_category.id", ondelete="CASCADE"), nullable=False
    )
    ...
    workout_items = relationship("WorkoutItems", back_populates="exercise")
    log_items = relationship("WorkoutLogItems", back_populates="exercise")
...
class WorkoutPlans(Base):
    ...
    scheduled = relationship("ScheduledWorkout", back_populates="plan")
...
class WorkoutItems(Base):
    ...
    sets: Mapped[int] = mapped_column(Integer, server_default=text("4"))
    reps: Mapped[str] = mapped_column(String(20), server_default=text("'10'"))
...
class ScheduledWorkout(Base):
    ...
    logs = relationship("WorkoutLog", back_populates="scheduled_workout")
...
class WorkoutLog(Base):
    ...
    ended_at: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=True))
